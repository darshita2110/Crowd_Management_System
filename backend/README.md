...
LWCC_HOME configuration
-----------------------

This backend may fall back to the `lwcc` Python package for inference when a
model isn't provided via `MODEL_PATH`. `lwcc` writes cache/config files under
the process home directory (usually `~/.lwcc`). In sandboxed or CI environments
the process HOME can be unset or pointed at `/`, which causes `lwcc` to try to
create `/.lwcc` and fail with a read-only filesystem error.

To control where LWCC writes, set the `LWCC_HOME` environment variable to a
writable directory before starting the server. The server will prefer this
location when present:

```bash
export LWCC_HOME=/path/to/some/writable/dir
uvicorn main:app --host 127.0.0.1 --port 8000
```

If `LWCC_HOME` is not set, the server will temporarily set `HOME` to a
temporary writable directory while importing/using LWCC to avoid writing to
`/`.

Testing notes
-------------
- If you run into async test errors like "async def functions are not natively
  supported", install `pytest-asyncio` in your environment:

```bash
pip install pytest-asyncio
```

 - Make sure `python-multipart` is installed if sending multipart form-data
   requests (Postman, curl):
...
Enjoy!
# Backend: Inference endpoint (quick test)

This document explains how to test the new `POST /inference/count` endpoint locally using Postman or curl.
...
python3 -m pip install python-multipart lwcc
```

Start server

```bash
cd /path/to/Crowd_Management_System/backend
uvicorn main:app --reload --host 127.0.0.1 --port 8000
```

Postman
1. Import `Crowd_Management_System.postman_collection.json` (exists in `backend/`).
2. Import `postman_environment.json` to set `base_url` to `http://127.0.0.1:8000`.
3. In the collection find `Inference -> Count People (Upload Image)`.
4. In the request Body tab select `form-data` and set keys:
   - `file` (type: File) — choose an image from `samplecrowd/` or your machine.
   - `save_record` (text) — optional `true`/`false`.
   - `radius_m` (text) — optional float to compute people_per_m2.
   - `event_id`, `area_name` (text) — optional metadata.
5. Send request and check Tests tab (there's a simple test asserting status 200 and numeric person_count).
...
curl -X POST "http://127.0.0.1:8000/inference/count" \
  ...
  -F "save_record=true" \
  -F "event_id=EV001" \
  -F "area_name=MainStage"
```

Notes
- If you see an error about multipart support, install `python-multipart`.
- If the endpoint reports "Could not run inference; no backend available", install `lwcc` or provide a torch model via `MODEL_PATH` environment variable.

Repository artifacts created by tests
- `backend/outputs/inference_postman_sample.json` — (generated by the smoke-run) contains the sample response produced by the TestClient run.
